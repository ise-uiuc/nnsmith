...
def analyze_lcov(path):
    ...
    for file_cov in file_covs:
        ...
        for cov_line in cov_lines[1:]:
            ...
            if cov_line.startswith('DA:'):
                ...
                if taken != '-' and taken != '0':
                    branches.add(line_number + ':' +
                                 block_number + ':' + branch_number)
                n_branch_total += 1
        ...
        assert len(
            lines) <= n_line_total, f'{len(lines)} <= {n_line_total} in {filename}'
        assert len(
            branches) <= n_branch_total, f'{len(branches)} <= {n_branch_total} in {filename}'
...
def analyze_folder(folder, redo=False, max_time=None):
    ...
    file_list = sorted(list(file_list), key=lambda x: int(
        str(x.name).split('.')[-3]))
    pool = multiprocessing.Pool(processes=min(
        len(file_list), multiprocessing.cpu_count()))
    ...
    def merge_cov(current, rhs, hint=''):
        ...
        for k in set(current.keys()).union(set(rhs.keys())):
            if k not in current:
                ...
                current[k]['lines'] = current[k]['lines'].union(
                    rhs[k]['lines'])
                ...
                if current[k]['lf'] != rhs[k]['lf']:
                    print(
                        f'[WARNING] total line {current[k]["lf"]} != {rhs[k]["lf"]} in {k} ' + hint)
                    current[k]['lf'] = max(current[k]['lf'], rhs[k]['lf'])

                if current[k]['bf'] != rhs[k]['bf']:
                    print(
                        f'[WARNING] total branch {current[k]["bf"]} != {rhs[k]["bf"]} in {k} ' + hint)
