...
if __name__ == '__main__':
    ...
    if args.lib:
        ...
        assert args.memcov, 'you need to provide either --lib for ast cov or --memcov for memcov.'

    if args.memcov and args.dp < 1000:
        print('==> For memcov, you need at least 1k data points as we lose cov when it crashes.')
    ...
    if not args.resume:
        ...
        assert os.path.exists(os.path.join(
            args.report_folder, 'stats.csv')), 'stats.csv does not exist!'
        import pandas as pd
        btimes = pd.read_csv(os.path.join(args.report_folder, 'stats.csv'), usecols=[
                             0], header=None).to_numpy().squeeze()
        process_time_sum = btimes.sum()

        history = open(os.path.join(args.report_folder,
                       'stats.csv'), 'r').readlines()
        if len(history) > 0 and not history[-1]:
            history.pop()

        next_batch_since_hist = int(
            history[-1].rstrip().split(',')[-1].split('.')[0]) + 1
            ...
            if not args.memcov:
                if os.path.exists(profraw_path):
                    ...
                    profdata_path = os.path.join(
                        args.report_folder, f'{i}.profdata')
