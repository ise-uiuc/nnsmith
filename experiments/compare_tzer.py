...
if __name__ == '__main__':
    ...
    parser.add_argument('--tzer', type=str, required=True,
                        help='Folder of tzer reports.')
    parser.add_argument('--nnsmith', type=str, required=True,
    ...
    tzer_unique = np.count_nonzero(
        np.logical_and(tzer_cov_bits, np.logical_not(np.logical_and(tzer_cov_bits, nnsmith_cov_bits)))
    )
    nnsmith_unique = np.count_nonzero(
        np.logical_and(nnsmith_cov_bits, np.logical_not(np.logical_and(tzer_cov_bits, nnsmith_cov_bits)))
    )
    print(f'Tzer Unique: {tzer_unique}')
    print(f'NNSmith Unique: {nnsmith_unique}')
    print(f'Common: {np.count_nonzero(np.logical_and(tzer_cov_bits, nnsmith_cov_bits))}')
