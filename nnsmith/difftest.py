...
if __name__ == "__main__":  # generate bug reports.
    ...
    def known_bug(report: dict, db: List[dict]):
        def same_model_same_stderr(report: dict):
            for last in reversed(db):
                if (
                    report["backend"] == last["backend"]
                    and report["model_idx"] == last["model_idx"]
                    and report["stderr"] == last["stderr"]
                ):
                    return True
                ...
                if (
                    report["stderr"] == "file not found"
                    and report["stdout"] == "file not found"
                ):
                    return True
            return False
        ...
        def trt_clip_int32(report: dict):
            return (
                report["backend"] == FactoryCreator.NAME_MAP["trt"]
                and "INVALID_NODE: Invalid Node - Clip" in report["stdout"]
            )

        def tvm_int_mismatch(report: dict):
            return (
                report["backend"].startwith("tvm")
                and "TypeError: mismatched types. int64 vs. int32" in report["stdout"]
            )

        def tvm_layout_argmin(report: dict):
            return (
                report["backend"].startwith("tvm")
                and "Invalid layout" in report["stdout"]
                and "(exist_axis[axis" in report["stdout"]
            )

        filters = [
            same_model_same_stderr,
            trt_clip_int32,
            tvm_int_mismatch,
            tvm_layout_argmin,
        ]
    ...
    def unsupported_feature(report: dict, db: List[dict]):
        def trt_round(report: dict):
            return (
                report["backend"] == FactoryCreator.NAME_MAP["trt"]
                and "getPluginCreator could not find plugin: Round version: 1"
                in report["stderr"]
            )
        ...
        def trt_general(report: dict):
            return (
                report["backend"] == FactoryCreator.NAME_MAP["trt"]
                and "UNSUPPORTED_NODE" in report["stdout"]
            )

        def xla_squeeze(report: dict):
            return (
                report["backend"] == FactoryCreator.NAME_MAP["xla"]
                and "BackendIsNotSupposedToImplementIt: Squeeze version 13 is not implemented"
                in report["stderr"]
            )
    ...
    def difftest(root: str):
        ...
        for model_folder in sorted(output_dir.glob("*/")):  # reproducibility
            ...
            def get_meta_info():
                ...
                num_in = len(
                    list(
                        (output_dir.parent / "model_input" / model_name).glob(
                            f"input.*.pkl"
                        )
                    )
                )
            ...
            for i in range(num_out):
                ...
                for backend in bknd_names:
                    ...
                    try:
                        ...
                        if isinstance(err, (NumericError, IncorrectResult)) and (
                            infer_succ is False or oracle_infer_succ is False
                        ):
                        ...
                        item = {
                            ...
                            if Path(out_path + ".stdout").exists()
                            else "file not found",
                            "stderr": open(out_path + ".stderr").read()
                            if Path(out_path + ".stderr").exists()
                            else "file not found",
                        }
