...
def locate_crash_testcase(batch_path):
    os.path.basename
    idx = [int(Path(i).stem) for i in glob.glob(os.path.join(batch_path, "*.onnx"))]
    idx = sorted(idx)[0]  # get the first one
    return (
        os.path.join(batch_path, f"{idx}.onnx"),
        os.path.join(batch_path, f"{idx}.pkl"),
    )
...
def simple_bug_report(
    ...
    if os.path.exists(onnx_external_data_dir):
        shutil.move(onnx_external_data_dir, onnx2external_data_dir(onnx_path_in_report))

    graph_path = buggy_onnx_path[: -len(".onnx")] + "-graph.pkl"
    if os.path.exists(graph_path):
        G = pickle.load(open(graph_path, "rb"))
        nx.drawing.nx_pydot.to_pydot(G).write_png(os.path.join(dir, "graph.png"))
...
class Reporter:  # From Tzer.
    def __init__(
        self, report_folder=None, lib_path=None, name_hint="", yes=False
    ) -> None:
        # Checks
        ...
        if os.path.exists(self.report_folder):
            ...
            if decision.lower() == "n":
                ...
                for f in Path(self.report_folder).glob("*"):
                    if str(f.name) not in [
                        "stdout.log",
                        "stderr.log",
                    ] and not f.name.endswith(".profraw"):
                        os.system(f"rm -rf {f}")
        ...
        if self.lib_path is not None:  # Enabling coverage tracing
            ...
            self.lcov_config = open(os.path.join(self.report_folder, "stats.csv"), "w")
    ...
    def report_bug(
        ...
        G = pickle.load(open(graph_path, "rb"))
        nx.drawing.nx_pydot.to_pydot(G).write_png(
            os.path.join(self.report_folder, dir, "graph.png")
        )
        shutil.move(
            buggy_onnx_path, os.path.join(self.report_folder, dir, "model.onnx")
        )
        if buggy_torch_path is not None:
            shutil.move(
                buggy_torch_path, os.path.join(self.report_folder, dir, "model.pt")
            )
        shutil.move(stdout, os.path.join(self.report_folder, dir, "stdout.log"))
        shutil.move(stderr, os.path.join(self.report_folder, dir, "stderr.log"))
        for i in Path(".").glob("mlist.*"):
            shutil.move(str(i), os.path.join(self.report_folder, dir, i.stem))
        ...
        if sat_inputs is not None:
            pickle.dump(
                sat_inputs,
                open(os.path.join(self.report_folder, dir, "sat_inputs.pkl"), "wb"),
            )
    ...
    def handle_profraw(self, profraw_path, n_models, time_spent):
        ...
        if os.path.exists(profraw_path):
            ...
            if 0 != os.system(
                ...
                f"{llvm_cov} export -instr-profile={profdata_path} -format=lcov {self.lib_expr} > {lcov_path}"
            ):
...
class CustomProgress(Progress):
    def __init__(
        self, fuzz_status, columns: List[Union[str, ProgressColumn]], disable=False
    ):
...
class FuzzingLoop:  # TODO: Support multiple backends.
    def __init__(
        ...
        self.root = root
        self.reporter = Reporter(report_folder=root, name_hint=str(factory), yes=yes)
    ...
    def rich(self):
        def make_panel(title, time_arr: np.ndarray):
            ...
            return Panel.fit(
                f"[green]Fast: {time_arr.min():.3f}s[/green]|"
        ...
        panels = [
            ...
            Panel.fit(
                f'{self.n_bug}/{len(self.rich_profile["succ_gen"])}',
                title="Bug/Iter",
                style="magenta",
                width=16,
            ),
    ...
    def testcase_gen(self, path, seed):
        ...
        self.rich_profile["inp_gen"] = np.append(
            self.rich_profile["inp_gen"], [time.time() - gen_tstart - gen_time]
        )
        ...
        with torch.no_grad():
            ...
            test_inputs = (
                sat_inputs
                if sat_inputs
                else net.get_random_inps(use_cuda=self.use_cuda)
            )
        ...
        return (
            {ina: inp for ina, inp in zip(inames, inputs)},
            {oname: outputs[i] for oname, i in zip(onames, oidx)},
            sat_inputs is not None,
        )
    ...
    def batch_add(self, onnx_path, oracle_path, force=False):
        ...
        if onnx_path is not None and os.path.exists(onnx_path):
            target_onnx = os.path.join(self.batch_path, f"{len(self.eval_batch)}.onnx")
            target_oracle = os.path.join(self.batch_path, f"{len(self.eval_batch)}.pkl")
        ...
        if (len(self.eval_batch) == self.eval_freq or force) and len(
            self.eval_batch
        ) > 0:
            # Execute batch evaluation
            copied_env = os.environ.copy()
            # Path to store llvm profile.
            profraw_path = os.path.join(self.root, f"{self.executed_batches}.profraw")
            ...
            self.n_bug = len(
                glob.glob(os.path.join(self.reporter.report_folder, "bug-*"))
            )
                ...
                while time.time() - start_time < self.time_budget:
                    ...
                    try:
                        with warnings.catch_warnings():  # just shutup.
                            ...
                            inps, outs, numvalid = self.testcase_gen(onnx_path, seed)
                        ...
                        progress.update(task_fuzz, completed=time.time() - all_tstart)
                        continue
                    # =================================
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inp_gen", type=str, help="default is None. Can be hybrid.")
    parser.add_argument("--gen_timeout", type=int, default=1000, help="in milliseconds")
    parser.add_argument(
        "--eval_freq",
        type=int,
        default=1,
        help="(EVALUATION ONLY) for batch processing",
    )
    ...
    if Path(cache_file).exists():
        ...
        print(
            f"Warning: Op dtypes config file `{cache_file}` does not exist. "
